...
df_t=df;
#----detecting 8 min in advance---
sign = lambda x: (1, -1)[x < 0]

def curve_shift(df_t, shift_by):
    '''
    ...
    df_t.insert(loc=0, column=labelcol+'tmp', value=vector)
    # Remove the rows with labelcol == 1.
    df_t=df.drop(df[df[labelcol] == 1].index)
    # Drop labelcol and rename the tmp col as labelcol
    df_t=df_t.drop(labelcol, axis=1)
    df_t=df_t.rename(columns={labelcol+'tmp': labelcol})
    # Make the labelcol binary
    df_t.loc[df[labelcol] > 0, labelcol] = 1

    return df_t
#------------remove time and categorial algorithms----------
...
n_in = len(X_valid_scaled);
epochs = 200;
batch = 64;
lr = 0.0001;
model1= Sequential()
#Encoder
model1.add(LSTM(4, activation='relu', input_shape=(timesteps, n_features), return_sequences=True));
...
'''lstm_autoencoder_history=model1.fit(X_train_y0_scaled, X_train_y0_scaled, 
                                                epochs=epochs, 
                                                batch_size=batch, 
                                                validation_data=(X_valid_y0_scaled, X_valid_y0_scaled),
                                                verbose=2)'''
lstm_autoencoder_history=model1.fit(X_train_y0_scaled, X_train_y0_scaled, epochs=300, verbose=0);
print('----- Loss----')
print(lstm_autoencoder_history['loss']);


#-------------plotting the loss in change----------
