...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/Aura_MLS/AURA_DATA_April_2014/final_tst_data_07_04_2014.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
#---------------------CNN model----------------------
model=Sequential()
model.add(Conv2D(32, kernel_size=(8, 8),activation='linear',input_shape=(128,64,4),padding='same'))
model.add(LeakyReLU(alpha=0.001));
model.add(MaxPooling2D((2, 2),padding='same'));
model.add(Conv2D(64, (8, 8), activation='linear',padding='same'));
model.add(LeakyReLU(alpha=0.001));
model.add(MaxPooling2D(pool_size=(2, 2),padding='same'));
model.add(Conv2D(128, (8, 8), activation='linear',padding='same'));
model.add(LeakyReLU(alpha=0.001))     ;             
model.add(MaxPooling2D(pool_size=(2, 2),padding='same'));
...
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, 1+Y,spe_metric,cmap='hsv')
plt.colorbar(cp)
#plt.clim(0, 1e-11)
...
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, Y,T_square,cmap='hsv')
plt.colorbar(cp)
#plt.clim(0, 1e-11)
