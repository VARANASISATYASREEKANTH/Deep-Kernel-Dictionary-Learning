...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/Aura_MLS/09_01_2015/final_tst_data_09_01_2015.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, 10+Y,spe_metric,cmap='hsv')
plt.colorbar(cp)
#plt.clim(0, 1e-11)
...
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, 10+Y,T_square,cmap='hsv')
plt.colorbar(cp)
#plt.clim(0, 1e-11)
