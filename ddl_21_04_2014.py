...
test_data_dash_pert1=np.zeros((sheet.nrows,sheet.ncols));
test_data_dash_pert=np.zeros((sheet.nrows,sheet.ncols));
for i in range(sheet.nrows):
    for j in range(sheet.ncols):
        test_data_dash_pert1[i][j]=sheet.cell_value(i,j)

for i in range(sheet.nrows):
    for j in range(sheet.ncols):
        test_data_pert[i][j]=test_data_dash_pert1[i][j]-np.mean(test_data_dash_pert1[i][1:60]);
...
'''
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/test_data/21_oct_1998/dictionary_cnn_21_10_1998_preprocessed.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
cnn_dict=np.zeros((sheet.nrows,sheet.ncols));

for i in range(sheet.nrows):
    for j in range(sheet.ncols):
        cnn_dict[i][j]=sheet.cell_value(i,j);
tst_data=np.zeros((128,128));
for i in range(1,128,1):
    for j in range(1,128,1):
        tst_data[i][j]=test_data_dash_pert1[i][j]
#-------------
Y1=np.matmul(cnn_dict,tst_data);
...
cp = plt.contourf(X, Y,eig_vec,cmap='jet')
plt.colorbar(cp)
plt.clim(0, 0.8)
ax.set_title('' )
