...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/test_data_08_05_1998_ver2.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
keras.utils.plot_model(model, "CNN_model_rayleigh_lidar1.pdf", show_shapes=True); 
...
keras.utils.plot_model(model_lstm, "LSTM_model_rayleigh_lidar1.pdf", show_shapes=True);    

#______________________LSTM 2 time________________ 
y_out_lstm_2=np.zeros((128,128));

out=np.zeros((128,128));
for i in range(1,128,1):
    out[1][:]=np.transpose(y_out_lstm[:][i])
        #------------define input sequence----------------
    seq_in=np.abs(out[1][:]);
    #-----reshape input into [samples, timesteps, features]---
    n_in = len(seq_in)
    ...
    decoder2 = TimeDistributed(Dense(1))(decoder2)
    #-----------tie it together-------------
    model_lstm_2= Model(inputs=visible, outputs=[decoder1, decoder2])
    model_lstm_2.compile(optimizer='adam', loss='mse',metrics=['accuracy','mse', 'mae', 'mape'])
    plot_model(model_lstm_2, show_shapes=True, to_file='composite_lstm_autoencoder.png')
    #-------------fit model-----------
    model_hist_lstm=model_lstm_2.fit(seq_in, [seq_in,seq_out], epochs=10, verbose=0)
    #-----------demonstrate prediction--------
    yhat_lstm_2= model_lstm_2.predict(seq_in, verbose=1);
    
    k=np.zeros(128);
    for j in range(0,127,1):
        k[j]=yhat_lstm_2[0][0][j][0];
        print(k[j])
    y_out_lstm_2[i][:]=k;
...
cp = plt.contourf(X, Y, y_out_lstm_2,cmap='jet')
plt.colorbar(cp)
#plt.clim(0, 0.015)
ax.set_title('FFT' )
