...
loc1=('G:/research_works_vssreekanth_jrf/MY_PAPERS/paper_3a_deep_learning_for_parameter_estimation/programs/anamoly_detection_LSTM_RNN/test_data__03_2000_ver2.xlsx')
wb = xlrd.open_workbook(loc1)
sheet = wb.sheet_by_index(0)
...
seq_model.add(ConvLSTM2D(filters=64, kernel_size=(2, 2),
...
seq_model.add(Conv3D(filters=1, kernel_size=(2, 2, 2),
               activation='sigmoid',
               padding='same', data_format='channels_last'))
seq_model.compile(optimizer='adam', loss='mse',metrics=['accuracy','mse', 'mae', 'mape'])
seq_model.summary();    
keras.utils.plot_model(seq_model, "ConvLSTM_model_rayleigh_lidar_2.pdf", show_shapes=True);    
y=np.zeros((128,26));
print(y);
y=y.reshape(1,26,1, 128, 1);
model_hist=seq_model.fit(data, y,epochs=20);
model_loss_train=seq_model.history;
model_metrics_train=seq_model.metrics_names;
#print(model_metrics);
yhat_ConvLSTM= seq_model.predict(data_test, verbose=1);
yhat_ConvLSTM=yhat_ConvLSTM.reshape(128,26);
