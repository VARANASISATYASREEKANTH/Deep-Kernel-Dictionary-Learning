...
for i in range(1,128,1):
    ...
    for j in range(1,128,1):
        SPE[i][j]=np.power(y_out_lstm[i][j]-y_out[i][j],2)/var_train[i];
#---T2 metric---
...
for t in range(100,103,1):
    ...
    [q,f,patches] = plt.hist(SPE[t][1:128], 128, facecolor='b',density=True, alpha=1,edgecolor = 'black');
    #---selection of optimal bandwidth---
    opt_mean=np.mean(SPE[100][1:128]);
    opt_std=np.power(np.var(SPE[t][1:128]),0.5);
    s=60;# no  of samples
...
#---thresholding---
Th_SPE=quantile(tot_density, 0.99);
Th_T_square=quantile(tot_density1, 0.99);
#--- Determining max index values with  confidence intervels
