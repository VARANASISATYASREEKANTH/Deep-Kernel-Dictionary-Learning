...
fig = plt.figure(figsize=(10,8))
plt.figure(4)
for t in range(100,128,1):
    #---------computing the overall threshold---
    [q,f,patches] = plt.hist(SPE[t][1:60], 60, facecolor='m',density=True, alpha=1,edgecolor = 'black');
    #---selection of optimal bandwidth---
    opt_mean=np.mean(SPE[t][1:60]);
    opt_std=np.power(np.var(SPE[t][1:60]),0.5);
    s=60;# no  of samples
    ...
    plt.plot(np.exp(log_density),linewidth='4');
