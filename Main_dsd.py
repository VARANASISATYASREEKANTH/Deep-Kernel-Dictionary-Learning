...
training_data= np.zeros(shape=(515,14))
loc_d = ('G:/mie_rayleigh_lidar_data/rayleigh_data/981021/integrated_photon_count_1998_10_21.xlsx')
wb = xlrd.open_workbook(loc_d)
sheet = wb.sheet_by_index(0)
sheet.cell_value(0, 0)
for k in range(1,14):#columns
    for i in range(0,515):#rows
       training_data[i][k]=sheet.cell_value(i,k);
#--------------reading testing  data-------------------------
test_data= np.zeros(shape=(515,14))
loc_d = ('G:/research_works_vssreekanth_jrf/MY_PAPERS/sparsity_including_norms_on_frame_expansion/double_sparsity_dictionary_learning/programs/double_sparsity_dictionary_lerning/recons_after_idwt_30_jan_2014.xlsx')#test data
wb = xlrd.open_workbook(loc_d)
sheet = wb.sheet_by_index(0)
sheet.cell_value(0, 0)
for k in range(1,14):#columns
    for i in range(1,515):#rows
       test_data[i][k]=sheet.cell_value(i,k);



noisy_data = np.random.normal(0, sigma ^ 2, (515,14))+test_data;
