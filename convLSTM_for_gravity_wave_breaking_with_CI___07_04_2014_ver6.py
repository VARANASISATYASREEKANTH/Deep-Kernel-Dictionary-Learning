...
norm_eig_vec=preprocessing.normalize(eig_vec_abs);
import numpy as np
...
plt.colorbar(cp)
plt.clim(threshold_eig, 1)
ax.set_title('21/04/2014,Threshold=0.3275' )
...
for t in range(100,103,1):
    ...
    opt_mean=np.mean(SPE1[100][1:128]);
    opt_std=np.power(np.var(SPE1[t][1:128]),0.5);
    s=60;# no  of samples
    ...
    plt.hexbin(f[0:128],np.exp(log_density),bins=50,cmap=plt.cm.hsv)
...
for t in range(1,128,1):
    ...
    #---selection of optimal bandwidth---
    opt_mean=np.mean(T_square[t][1:60]);
    opt_std=np.power(np.var(T_square[t][1:128]),0.5);
    s=60;# no  of samples
...
y_vals = np.linspace(start1, stop1, n_values1)
X, Y=np.meshgrid(x_vals, y_vals)
cp = plt.contourf(X, Y,T_square,cmap='jet')
plt.colorbar(cp)
#plt.clim(threshold_eig, 1)
...
data = np.random.multivariate_normal((0, 0), [[0.8, 0.05], [0.05, 0.7]], 12)
x=T_square[1:128][1];
y=np.transpose(T_square[1][1:128]); 
xmin, xmax = conf_int_T_square[0],conf_int_T_square[1];
ymin, ymax =conf_int_T_square[0],conf_int_T_square[1];
# Peform the kernel density estimate
xx, yy = np.mgrid[-7.239974915556927e-05:7.309381454980639e-05:1.31e-5,-7.239974915556927e-05:7.309381454980639e-05:1.31e-5 ];
...
# Contourf plot
cfset = ax.contourf(xx, yy, f, cmap='hsv');
